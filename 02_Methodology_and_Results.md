...
| 5b. Techno-economic analysis — CAPEX & payback (Python) | Complete |
| 5c. Sensitivity analysis (Python) | Complete |
...
**Tool:** Python, see `sensitivity_analysis.py`. Each variable is swept individually (+/-20% for electricity and gas price, +/-30% for CAPEX given the wider literature uncertainty), holding the others at the base case, to show which assumption the payback result actually depends on most.

| Variable | Payback at low input | Payback at high input | Swing |
|---|---|---|---|
| Gas price | 4.0 yr (at -20%) | 2.2 yr (at +20%) | 1.78 yr |
| CAPEX | 2.0 yr (at -30%) | 3.7 yr (at +30%) | 1.70 yr |
| Electricity price | 2.6 yr (at -20%) | 3.1 yr (at +20%) | 0.50 yr |

**Finding:** gas price is the single biggest driver of payback uncertainty, followed closely by CAPEX, with electricity price mattering considerably less. This makes sense once examined: gas price affects both sides of the comparison at once (the baseline case and the residual top-up in the retrofit case), giving it outsized leverage over the result. Across the entire tested range, payback never exceeds 4 years, reinforcing that the investment case is robust, not fragile to any single assumption.
